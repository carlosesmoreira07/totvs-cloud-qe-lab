...
export const aiExecutiveScorecardSchema = z.object({
  ...
  affectedRisks: z.array(executiveScorecardFindingSchema).max(5),
  strongestEvidence: z.array(executiveScorecardFindingSchema).max(5),
  regressions: z.array(executiveScorecardFindingSchema).max(5),
  degradedJourneys: z.array(executiveScorecardFindingSchema).max(5),
  resilienceFindings: z.array(executiveScorecardFindingSchema).max(5),
  observabilityFindings: z.array(executiveScorecardFindingSchema).max(5),
  performanceFindings: z.array(executiveScorecardFindingSchema).max(5),
  coverageGaps: z.array(executiveScorecardFindingSchema).max(5),
  recommendedInvestigations: z.array(executiveScorecardFindingSchema).max(5),
  recommendedTests: z.array(executiveScorecardFindingSchema).max(5),
  humanQuestions: z.array(executiveScorecardFindingSchema).max(3),
